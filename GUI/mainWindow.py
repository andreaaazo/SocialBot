...
class mainWindow(Tk):
	def __init__(self):
		...
		self.title("Socialbot by @andreaaa.zo")
		...
		for i in self.x:  # create menu commands account stored in dict
			self.accountManagerMenu.add_command(
				label="Account " + str(i),
				command=lambda i=i: show_account(i)
			)

		# Proxies Menu
		self.mainMenu.add_cascade(
			label="Proxies",
			menu=self.proxiesMenu
		)

		self.proxiesMenu.add_command(
			label="Free Proxy",
			command=show_free_proxy
		)

		self.proxiesMenu.add_command(
			label="User Proxy",
			command=show_user_proxy
		)

		# Bot Settings Menu
		self.mainMenu.add_cascade(
			label="Bot",
			menu=self.botSettingsMenu
		)

		self.botSettingsMenu.add_command(
			label='Settings',
			command=show_bot_settings
		)